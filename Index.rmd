...
We loaded the libraries for reading and analyzing the data.
...
Next we import the dataframe

```{r read_data, message = FALSE, cols.print = 15}
...
-   PCO2: Partial Pressure of Carbon Dioxide, this measures the
    carbondioxide within the arterial or venous blood.
...
10.   how many patients in the hospital with Hperlipemia are dead?

11.   how many patients in the hospital with Anemia are dead?

12.   What is the proportion of survival and non-survival between diabetic
    and non-diabetic patients

13.  What is the proportion of survival and non-survival between
...
### The Data Structure
```{r data_structure,message=FALSE, results='hide'}
str(health_rec) # data structure
```
At first glance, all the variables are numeric type (not shown here, due to the length). There are `r dim(health_rec)[1]` observations and `r dim(health_rec)[2]` variables. Some of the variables of the data does not follow the data definition. A quick descriptive summary of the data shows this.
...
### Missing values
Next, we investigate the data for missing values. It is alright to have some missing values as regards some variables, but the outcome should be known. Outcome will be used to handle the missing data
```{r}
health_rec2%>%
...
Patients that are **89 years** of age are the highest.

### Question 1b: Which age group is the most in the hospital?

```{r}
breaks <- seq(from = 0, to = 100, by = 20) #this starts 
#at the lowest age and ends at the lowest
  
age_interval <- cut(health_rec$age, breaks = breaks,
                    right = TRUE,
                    include.lowest = TRUE,
                    ordered_result = TRUE) # this makes the class interval from the list
