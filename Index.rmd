...
```{r}
health_rec_test <- health_rec_test %>%
  mutate(log_pred = ifelse(predict(logistic_model, health_rec_test) >= 0.5, 1, 0))
```

```{r}
mean(health_rec_test$outcome == health_rec_test$log_pred)
```
The logistic model is having a 87.3% accuracy. Next we will see a confusion matric to see how well it performs
```{r}
table(log_predicted = health_rec_test$log_pred, actual = health_rec_test$outcome)
```
...
The best parameter for ntree = 700 will be used for the model.
...
```{r}
pred <- ifelse(predict(rf_model, health_rec_test) <= 0.5, 0, 0)

table(actual = health_rec_test$outcome, predicted = pred)
```
