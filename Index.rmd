...
numerical <- health_rec %>%
  select(-colnames(categorical), -c(1:4, 52))

str(numerical)
```
We are going to impute the missing values in the numerical variables using their mean.

```{r}
# Create functions to find mean removing the NA's
mean_without_na <- function(x){
  mean(x, na.rm = T)
}

# Pass the function to an apply statement to get individual column mean
num_val_means <- lapply(numerical, FUN = mean_without_na)
```

```{r}
impute_mean <- function(x,y){
  ifelse(is.na(x), num_val_means[[y]], x)
}
```

```{r}
health_rec %>%
  mutate(BMI = impute_mean(BMI, 1),
         heart_rate = impute_mean(heart_rate, 2),
         Systolic_blood_pressure = impute_mean(Systolic_blood_pressure, 3),
         Diastolic_blood_pressure = impute_mean(Diastolic_blood_pressure, 4),
         Respiratory_rate = impute_mean(Respiratory_rate, 5),
         temperature = impute_mean(temperature, 6),
         SP_02 = impute_mean(SP_02, 7),
         Urine_output = impute_mean(Urine_output, 8),
         hematocrit = impute_mean(hematocrit, 9),
         RBC = impute_mean(RBC, 10))
```

```{r}
impute_means <- function(data, columns_to_impute) {
  for (i in 1:length(columns_to_impute)) {
    col_name <- columns_to_impute[i]
    data <- data %>%
      mutate(!!col_name := impute_mean(!!sym(col_name), i))
  }
  return(data)
}
...
## Question 2
which age_group of the different gender died the most from data collected?
...
health_rec %>%
  ...
  ggplot(aes(age_group, dead_patients, fill = gender)) +
...
The result shows the frequency table for dead patients according to age group and gender. The age group 60 to 80 years have the count for both genders.
...
health_rec %>%
  ...
  count()
```
Female have the larger population in the clinic.
