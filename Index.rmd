...
str(numerical)
```
We are going to impute the missing values in the numerical variables using their mean. We use the e1071 package impute function here.
...
Next we replace the old values with new ones in the main data frame.
```{r}
health_rec_clean <- health_rec %>%
  select(-c(names(numerical))) %>%
  bind_cols(imputed_means)
```

Now we check for missing values to confirm
```{r}
...
```
```{r}
# Change the age_group name to remove brackets
health_rec_clean <- health_rec_clean %>%
  mutate(age_group =  ifelse(age_group == "[0,20]", "0-20",
                             ifelse(age_group == "(20,40]", "20-40",
                                    ifelse(age_group == "(40,60]", "40-60",
                                           ifelse(age_group == "(60,80]", "60-80", "80-100")))),
